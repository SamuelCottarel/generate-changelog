...
exports.getCommits = function (options) {
  ...
  .map(function (raw) {
    ...
    if (commit.subject.toLowerCase().includes('breaking change') | commit.subject.toLowerCase().includes('breaking changes') | commit.subject.toLowerCase().includes('breaking')) {
      commit.flagIfBreaking = commit.type;
