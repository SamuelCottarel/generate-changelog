...
exports.markdown = function (version, commits, options) {
  ...
  .each(function (type) {
    ...
    type === 'breaking' ? content.push('### ' + typeDescription + ' :warning:') : content.push('##### ' + typeDescription);
    content.push('');
    ...
    Object.keys(this.types[type]).forEach(function (category) {
      ...
      types[type][category].forEach(function (commit) {
        ...
        var breaking = type === 'breaking' ? ' :warning: <span style="color:red">**BREAKING CHANGE**</span>: ' : ' ' + (commit.flagIfBreaking ? '(' + commit.flagIfBreaking + '):' : '');

        content.push(prefix + breaking + subject.trim() + ' (' + shorthash + ')' + (type === 'breaking' && body ? '\n  * ' + body : ''));
      });
    });
